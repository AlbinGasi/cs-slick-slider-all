...
class cs_slick_all_bootstrap {
	...
	function slick_slider_all_shortcode($atts) {
		...
		$current_page = $_SERVER['SCRIPT_URL'];
		$current_page = str_replace('/', '', $current_page);
		
		$current_options = get_option('cs_slick_slider_opt');
		
		#echo '<pre>';
		#print_r($current_options);
		#echo '</pre>';
		
		if ( isset($current_options[$current_page]) ) {
			// already exist
			$get_option_value = $current_options;
			$get_option_value = $get_option_value[$current_page];
			
			$count = (int)$get_option_value + 1;
			
			$current_options[$current_page] = $count;
			
			update_option('cs_slick_slider_opt', $current_options);
		} else {
			$set_option = [$current_page => '1'];
			
			$new = $this->array_push_assoc($current_options, $current_page, $set_option);
			update_option('cs_slick_slider_opt', $new);
		}
		...
		$slider_id = get_option('cs_slick_slider_opt');
		$slider_id = $slider_id[$current_page];
